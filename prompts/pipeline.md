...
- 凭据：本仓库根目录 `config.json`，支持多账号（`default` + `accounts` 字典）；
- 目标公众号：默认用 `default` 账号，要用别的账号时在第三步命令加 `--account <账号名>`。
...
## 第二步：取标题和摘要
读生成的 `.md`：
- 标题：正文第一行 H1，只去掉行首的 Markdown 标记（`# `），其余原样作为文章标题；不要自行删改任何后缀、竖线或系列名；
